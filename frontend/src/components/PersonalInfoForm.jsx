...
const PersonalInfoForm = ({data={}, onChange, removeBackground, setRemoveBackground}) => {
  ...
  return (
    <div>
        <h3 className='text-lg font-semibold tex-grat-900'>Personal Information</h3>
        ...
        <div className='flex items-center gap-2'>
            <label>
                {data.image ? (
                    ...
                    <div className='inline-flex items-center gap-2 mt-5 text-slate-600 hoer:text-slate-700 cursor-pointer'>
            ...
            {typeof data.image === 'object' && (
                <div className='flex flex-col gap-1 pl-4 text-sm'>
                    ...
                    <label  className='relative inline-flex items-center cursor-pointer text-gray-900 gap-3'>
                        ...
                        checked={removeBackground} onChange={(e)=>{setRemoveBackground(e.target.checked)}}/>
        ...
        {fields.map((field)=>{
            ...
            return(
                <div key={field.key} className='space-y-1 mt-5'>
                    ...
                    <input type={field.type} value={data[field.key] || ""} onChange={(e)=>{handleChange(field.key, e.target.value)}} className='mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring focus:ring-green-500 focus:border-green-500 outline-none traition-colors text-sm' placeholder={`Enter your ${field.label.toLowerCase()}`} required={field.required}/>
                </div>
            )
        })}
    </div>
